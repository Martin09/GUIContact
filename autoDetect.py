...
class ImageObject(object):
    def __init__(self, image, bitTolerance=None, descriptor=Descriptor(), crop_x=(0, -1), crop_y=(0, -1),
                 bitx=[(1, 0), (2, 0), (3, 0), (0, 1), (1, 1), (2, 1), (3, 1)],
                 bity=[(2, 3), (1, 3), (0, 3), (3, 2), (2, 2), (1, 2), (0, 2)]):
        ...
        if isinstance(image, str) or isinstance(image, unicode):
            ...
            if len(self.__IMG_input__.shape) == 3:
                self.__IMG_gray__ = cv2.cvtColor(self.IMG_input, cv2.cv.CV_BGR2GRAY)[crop_y[0]:crop_y[1],
                                    crop_x[0]:crop_x[1]]
    ...
    def getTrimmed(self, image, cutoff=3):
        ...
        factx = abs(
            (self.bits.max() + 1) * self.descriptor.bitDistance - self.descriptor.markerArea[0] / 2.) / self.scale
        facty = abs(
            (self.bits.max() + 1) * self.descriptor.bitDistance - self.descriptor.markerArea[1] / 2.) / self.scale
    ...
    def findMarkers(self, image=None, method='default', **kwargs):
        ...
        if method == 'HoughCircles':  # Using Hough transform to find circular contours
            ...
            circles = cv2.HoughCircles(gray, cv.CV_HOUGH_GRADIENT, 1, 65, param1=100, param2=14, minRadius=28,
                                       maxRadius=35)
    ...
    def findNanowires(self, image=None, minLength=3, maxLength=10):
        ...
        for contour in contours:
            ...
            if wire.Length * self.scale > minLength and wire.Length * self.scale < maxLength and wire.Length * self.scale < min(
                    self.descriptor.markerArea) and wire.Length > 2. * wire.Width:
    ...
    def calcTransformation(self, markers, **kwargs):
        ...
        for i in range(4):
            ...
            centers[i] = myRotate(unitVectors[i], a) * magnitudes[i] / 2. * np.sqrt(1. + (ba) ** 2) + markers[
                l[i]].Center
    ...
    def showMarkers(self, ax, color=[128, 0, 0], lw=1, ms=10, marker="+", cellFrame=False):
        ...
        if cellFrame:
            ...
            p = Rectangle(xy, width=self.descriptor.markerArea[0] / self.scale,
                          height=self.descriptor.markerArea[1] / self.scale,
    ...
    def showBitPositions(self, ax, color=[0, 0, 255]):
        ...
        for i, pos in enumerate(np.vstack((px, py))):
            ...
            p = Rectangle(xy, width=self.descriptor.bitDistance / self.scale,
                          height=-self.descriptor.bitDistance / self.scale,
