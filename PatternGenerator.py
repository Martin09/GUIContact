...
class EmptyDesc(object):
    def __init__(self, markerSize=4, markerArea=(200, 150), cellSize=(500, 500), cellsPerBlock=(16, 16),
                 centerMarkSize=100,
                 bitDistance=8, centerMark=True, noBlocks=(8, 8), noBitsX=4, noBitsY=4, text="MASK",
                 ebeamMarkerSize=20.,
...
class Descriptor(EmptyDesc):
    ...
    def getBlockPosition(self, block):
        pos = array([(block[i] - 0.5) * self.cellSize[i] * self.cellsPerBlock[i] - 0.5 * self.cellSize[i] *
                     self.cellsPerBlock[i] * self.noBlocks[i] for i in [0, 1]])
        return np.array(pos, dtype=np.int)
...
class Cell(Structure):
    ...
    def createFrame(self, details=2):
        ...
        if details == 1:
            ...
            CELLFRAME.insertElement(Path(
                [[wo / 2., Ho / 2], [-wo / 2., Ho / 2], [-wo / 2., -Ho / 2], [wo / 2., -Ho / 2], [wo / 2., Ho / 2]], 1),
                                    layer=self.descriptor.commentLayer)
            CELLFRAME.insertElement(Path(
                [[wi / 2., Hi / 2], [-wi / 2., Hi / 2], [-wi / 2., -Hi / 2], [wi / 2., -Hi / 2], [wi / 2., Hi / 2]], 1),
                                    layer=self.descriptor.commentLayer)
        elif details == 2:
            ...
            CELLFRAME.insertElement(CIRCLEMARKER,
                                    xy=(-self.descriptor.markerArea[0] / 2. + 3 * self.descriptor.bitDistance,
...
class Block(Cell):
    ...
    def createFrame(self, details=2):
        ...
        if details == 1:
            wi = self.descriptor.cellSize[0] * self.descriptor.cellsPerBlock[0]
            Hi = self.descriptor.cellSize[1] * self.descriptor.cellsPerBlock[1]
            BLOCKFRAME.insertElement(Path(
                [[wi / 2., Hi / 2], [-wi / 2., Hi / 2], [-wi / 2., -Hi / 2], [wi / 2., -Hi / 2], [wi / 2., Hi / 2]]),
                                     layer=self.descriptor.commentLayer)
        elif details == 2:
    ...
    def createPattern(self, details=2):
        if details in [0, 1]:
            ...
            zero = [-0.5 * cellSize[i] * cellsBlock[i] + (cellSize[i] - self.__desc__.markerArea[i]) / 2. for i in
                    [0, 1]]
            ...
            for i in range(maxBits):
                ...
                if i < len(bin(cellsBlock[0] - 1)) - 2:
                    ...
                    y2 = zero[1] + cellSize[1] * 2 ** i + (self.__desc__.noBitsY - (
                    i + 1) / self.__desc__.noBitsY - 1) * bitDist
                    dx2 = 0
