...
def rotMatrix(angle):
    return np.array([[cos(angle / 180. * np.pi), -sin(angle / 180. * np.pi)],
                     [sin(angle / 180. * np.pi), cos(angle / 180. * np.pi)]])
    ##TODO use this more
...
class Structure(Element):
    ...
    def calcExtents(self, xy=(0, 0), angle=0, mag=1, **kwargs):
        ...
        """
        rot = np.array([[cos(angle / 180. * np.pi), -sin(angle / 180. * np.pi)],
                        [sin(angle / 180. * np.pi), cos(angle / 180. * np.pi)]])
...
class Array(Element):
    ...
    def __getGDS__(self, lib, xy=(0, 0), layer=None, angle=0, mag=1, unitsfact=1E3, **kwargs):
        ...
        elem = Elements.ARef(self.__structure__.name, self.rows, self.cols,
...
class Text(Pattern):
    ...
    def show(self, ax, xy=(0, 0), angle=0, mag=1, layer=None, pltTransform=f, maxRegion=2., cmap=colorMap(), **kwargs):
        ...
        annotations = []
        for p in poly:
            a = p.show(ax, xy=(0, 0), angle=0, mag=1, layer=layer, pltTransform=pltTransform, maxRegion=maxRegion,
                       cmap=cmap)
            annotations.extend(a)
        return annotations
